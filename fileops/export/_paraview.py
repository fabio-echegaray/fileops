...
def export_paraview(cfg_vol: ConfigVolume, **kwargs):
    log.info(f"Exporting data from configuration file {cfg_vol.configfile} into a format that Paraview can read.")
    ...
    channels = dict()
    export_tiff_path = ensure_dir(cfg_vol.path / "tiff")

    vol_timeseries, ch_metadata = bioformats_to_tiffseries(cfg_vol=cfg_vol, save_path=export_tiff_path)
    ...
    for chkey, tr_fn in zip(ch_metadata.keys(), tfn_lst):
        ...
        channels[chkey] = {
            ...
            "position":            cfg_vol.image_file.image_path.name.split("_")[-1].split(".")[0],
            "session":             cfg_vol.image_file.image_path.parent.parent.name,
